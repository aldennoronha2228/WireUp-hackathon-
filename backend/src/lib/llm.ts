...
export async function callLLM(
  ...
  try {
    ...
    // 3. TokenLB with WU Lite as last resort
...
export async function streamTokenLB(
  ...
  try {
    ...
    // 2. Groq fallback (word-by-word simulated streaming)
    const gKeys = groqKeys();
    if (gKeys.length > 0) {
      try {
